...
<button id='roster-upload-btn' type='button'>Загрузить разнарядку</button>
""".strip()
...
(function () {
  function installRosterUpload() {
    ...
    input.addEventListener('change', async function () {
      ...
      button.textContent = 'Загрузка разнарядки...';
      ...
      try {
        ...
        alert(error && error.message ? error.message : 'Ошибка загрузки разнарядки');
      } finally {
...
def _safe_filename(original_name: str, suffix: str) -> str:
    ...
    stem = Path(original_name).stem.strip() or "raznaryadka"
